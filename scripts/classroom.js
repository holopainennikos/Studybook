  // Function to create the students dynamically
  function generateStudents() {
    const studentContainer = document.querySelector('.students');
    
    // Loop to generate 20 students
    for (let i = 1; i <= 20; i++) {
      const student = document.createElement('p');
      student.classList.add('student-image', 'draggable');
    ...
    // Once students are generated, initialize drag-and-drop events
  ...
  function initializeDragAndDrop() {
    ...
    classrooms.forEach(classroom => {
      ...
      classroom.addEventListener('dragover', e => {
        ...
        if (y < 0) y = 0; // Prevent dragging past the top edge
  ...
  // Call the function to generate students and enable dragging on page load
